...
struct Account {
    ...
    /*Total transaction fee over all executed order. Transaction fee is based on fee_config. It's totle quote amount.
    ...
    */
     ...
     * max_volume_ratio < limit order volume than limit order would be filled partially in this given OHLC tick.*/
    float max_volume_ratio = 0.0f;

    /* Initialize the account with config*/
    void init_account(const AccountConfig &account_confg);

    /* Fee in quote currency according to fee_config and given quote currency amount in the transaciton.*/
    float get_fee(const FeeConfig &fee_config, float quote_amount) const;

    /* Maximum tradeable base currency (BTC) amount (volume) based on max_volume_ratio on given ohlc_tick*/
    float get_max_base_amount(const OhlcTick &ohlc_tick) const;
    ...
    /* Buy as much of base currency(BTC) as possible at given price. Using most quote_amount (USD), It's possible to
     * buy most max_base_amount base.*/
    bool buy_at_quote(const FeeConfig &fee_config, float quote_amount, float price,
                      float max_base_amount = std::numeric_limits<float>::max());

    /* Sell amount of base currency at given price.*/
    bool sell_base_currency(const FeeConfig &fee_config, float base_amount, float price);

    /* Sell as much of base currency as possible at given price. Getting at most quote_ammout. It's possible to sell
     * max_base_amount at the price*/
    bool sell_at_quote(const FeeConfig &fee_config, float quote_amount, float price,
                       float max_base_amount = std::numeric_limits<float>::max());
    ...
    bool market_buy_at_quote(const FeeConfig &fee_config, const OhlcTick &ohlc_tick, float quote_amount);

    /* Execute market sell order of base_amount (BTC). (I want to sell 0.04 BTC)*/
    bool market_sell(const FeeConfig &fee_config, const OhlcTick &ohlc_tick, float base_amount);

    /* Execute market sell order receiving at most quote_amount. (I want to sell $100 worth of BTC)*/
    ...
    /* Execute stop buy order of quote_amount of quote currency at stop_price. Stop Buy only get executed when price
    ...
    /* Execute stop sell order of base_amount in base currency (BTC) at stop_price. Stop Sell only get executed when
    ...
    /* Execute stop sell order of quote_amount in quote currency (USD) at stop_price. Stop Sell only get executed when
    ...
    /* Execute limit buy order of base amount at limit_price. Limit Buy only get executed when price FALLS to
    ...
    /* Execute limit buy order of quote_amount of quote currency at limit_price. Limit Buy only get executed when price
    ...
    /* Execute limit sell order of base_amount in base currency (BTC) at limit_price. Limit Sell only get executed when
    ...
    /* Execute limit sell order of quote_amount in quote currency (USD) at limit_price. Limit Sell only get executed
