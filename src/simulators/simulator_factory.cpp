...
std::vector<std::unique_ptr<SimulatorDispatcher>> get_batch_of_rebalancing_trade_simulators() {
    return RebalancingSimulatorDispatcher::get_batch_of_simulator({0.1f, 0.3f, 0.5f, 0.7f, 0.9f},
...
std::vector<std::unique_ptr<SimulatorDispatcher>> get_batch_of_stop_trade_simulators() {
    return StopTradeSimulatorDispatcher::get_batch_of_simulator(/*stop_order_margins=*/{0.05, 0.1, 0.15, 0.2},
...
std::vector<std::unique_ptr<SimulatorDispatcher>> get_batch_of_simulators(std::string_view &strategy_name) {
    if (strategy_name == RebalancingTradeSimulatorName) {
        return get_batch_of_rebalancing_trade_simulators();
    } else if (strategy_name == StopTradeSimulatorName) {
        return get_batch_of_stop_trade_simulators();
    } else {
        std::exit(EXIT_FAILURE);
    }
}
